...
def simulationWithoutDrug(numViruses, maxPop, maxBirthProb, clearProb,
    ...
    virusList = [0] * 300
    timeSteps = 300
    for simulation in range(numTrials):
        viruses = []
...
def simulationWithDrug(numViruses, maxPop, maxBirthProb, clearProb, resistances,
    ...
    steps = 300
    eachTreatment = 150
    trialResultsTotal = [[] for s in range(steps)]
    trialResultsResistance = [[] for s in range(steps)]
    for __ in range(numTrials):
        viruses = [ResistantVirus(maxBirthProb, clearProb, resistances.copy(), mutProb)
                    for virus in range(numViruses)]
        patient = TreatedPatient(viruses, maxPop)
        for step in range(steps):
            if step == eachTreatment:
                patient.addPrescription("guttagonol")
            patient.update()
            trialResultsTotal[step].append(patient.getTotalPop())
            trialResultsResistance[step].append(patient.getResistPop(["guttagonol"]))
    resultsSummaryTotal = [sum(l) / float(len(l)) for l in trialResultsTotal]
    resultsSummaryResistance = [sum(l) / float(len(l)) for l in trialResultsResistance]
    pylab.plot(resultsSummaryTotal, label="Total Virus Population")
    pylab.plot(resultsSummaryResistance, label="Resistant Virus Population")
