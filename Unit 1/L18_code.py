...
def powerSet(items):
    N = len(items)
    for i in xrange(2**N):  # enumerate the 2**N possible combinations
        combo = []
        for j in xrange(N):  # test bit jth of integer i
            if (i >> j) % 2 == 1:
                combo.append(items[j])
        yield combo  # ipdb> p combo  [<__main__.Item object at 0x2c25150>]
...
def yieldAllCombos(items):
    ...
    """
    N = len(items)
    for i in xrange(3**N):  # enumerate the 2**N possible combinations
        bag1 = bag2 = []
        for j in xrange(N):  # test bit jth of integer i
            if (i / (3 ** j)) % 3 == 1:
                bag1.append(items[j])
...
t1 = timeChooseBest(14)
t2 = timeChooseBest(15)
# In [7]: 0.138/0.068
# Out[7]: 2.0294117647058822
# In [9]: 15/7.
# Out[9]: 2.142857142857143
